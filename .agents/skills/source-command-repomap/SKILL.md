...
description: "Refresh the repo map using parallel subagents - one per group in .Codex/repomap-config.json"
---

# source-command-repomap

Use this skill when the user asks to run the migrated source command `repomap`.

## Command Template

Refresh the FlowForm repo map using parallel subagents. Each agent handles a
group of related paths from the `agents` array in `.Codex/repomap-config.json`.

## Steps

1. Read `.Codex/repomap-config.json` to get the `agents` groups and `output_dir`.
