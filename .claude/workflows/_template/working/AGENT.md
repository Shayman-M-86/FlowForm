...
summarizing/compressing tool on files under `source/` or `working/`.
...
9. Write a pass report to `working/pass-reports/<N>-<slug>.md` using the
   template in `working/pass-template.md`
...
- Do not touch source files in `source/` — read only.
