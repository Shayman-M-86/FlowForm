...
@openapi_route(summary="List project members", response_model=list[ProjectMemberOut], tags=["Members"])
...
def list_members(project_id: int):
    ...
    return [ProjectMemberOut.model_validate(m).model_dump(mode="json") for m in members], 200
...
def list_invitations(project_id: int):
    ...
    return [ProjectInvitationOut.model_validate(i).model_dump(mode="json") for i in invitations], 200
...
def send_invitation(project_id: int):
    ...
    return ProjectInvitationOut.model_validate(invitation).model_dump(mode="json"), 201
