...
class FlowForm(BaseModel):
    ...
    def validate_cors_settings(self) -> FlowForm:
        ...
        if not self.cors.origins:
            raise ValueError(
                "FLOWFORM_CORS_ORIGINS must contain at least one explicit origin when FLOWFORM_ENV is staging or prod"
            )
        if self.cors.supports_credentials and "*" in self.cors.origins:
            raise ValueError(
                "FLOWFORM_CORS_ORIGINS must not contain '*' when credentials are enabled in staging or production"
            )
        return self

    @model_validator(mode="after")
    def validate_deployed_site_url(self) -> FlowForm:
        """Require an HTTPS frontend URL for links generated by deployed apps."""
        if self.env not in {"staging", "prod"}:
            return self

        parsed = urlsplit(self.server.site_url)
        if parsed.scheme != "https" or not parsed.hostname or parsed.hostname in {"localhost", "127.0.0.1"}:
            raise ValueError(
                "FLOWFORM_SERVER_SITE_URL must be an HTTPS frontend URL when FLOWFORM_ENV is staging or prod"
            )
        return self
