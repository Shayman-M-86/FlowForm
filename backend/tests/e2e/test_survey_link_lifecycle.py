...
def _links_url(seed: SeedData) -> str:
    return f"/api/v1/studio/projects/{seed.project.id}/surveys/{seed.survey.id}/links"


def test_create_link_populates_project_id(authed_client: FlaskClient, seed: SeedData, core_db_session: Session) -> None:
    ...
    assert link.project_id == seed.project.id, f"link.project_id={link.project_id!r}, expected {seed.project.id!r}"


def test_link_lifecycle_create_list_resolve_update_delete(authed_client: FlaskClient, seed: SeedData) -> None:
    ...
    resolve = authed_client.post("/api/v1/respondent/links/resolve", json={"token": token})
