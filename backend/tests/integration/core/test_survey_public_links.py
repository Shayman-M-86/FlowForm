...
def test_survey_public_link_can_be_created(
    ...
    """All fields are persisted and server defaults populate is_active, allow_response, and created_at."""
    ...
    saved = db_session.get(SurveyPublicLink, link.id)
    assert saved is not None, "SurveyPublicLink was not persisted"
    ...
    assert saved.allow_response is True, f"allow_response={saved.allow_response!r}, expected True from server default"
...
def test_survey_public_link_unique_token_hash(
    ...
    assert constraint == "uq_survey_public_links_token_hash", (
        f"Expected constraint 'uq_survey_public_links_token_hash', got '{constraint}'\n"
...
def test_survey_public_link_unique_prefix_within_survey(
    ...
    assert constraint == "uq_survey_public_links_survey_id_token_prefix", (
        f"Expected constraint 'uq_survey_public_links_survey_id_token_prefix', got '{constraint}'\n"
        f"DB error: {exc_info.value}"
    )
...
def test_survey_public_link_rejects_short_token_prefix(
    ...
    constraint = orig.diag.constraint_name
    assert constraint == "ck_survey_public_links_token_prefix_len", (
        f"Expected constraint 'ck_survey_public_links_token_prefix_len', got '{constraint}'\n"
        f"DB error: {exc_info.value}"
    )
...
def test_survey_public_link_rejects_short_token_hash(
    ...
    orig = cast(CheckViolation, exc_info.value.orig)
    constraint = orig.diag.constraint_name
    assert constraint == "ck_survey_public_links_token_hash_len", (
        f"Expected constraint 'ck_survey_public_links_token_hash_len', got '{constraint}'\n"
        f"DB error: {exc_info.value}"
    )
...
def test_survey_public_link_cascades_on_survey_delete(
    ...
    assert db_session.get(SurveyPublicLink, link_id) is None, (
        "SurveyPublicLink should have been deleted when its survey was deleted"
    )
