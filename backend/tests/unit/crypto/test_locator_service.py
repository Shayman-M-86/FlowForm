...
from app.crypto.linkage_key_service import LinkageKey, LinkageKeyService
...
def _make_service(
    ...
    keys.get_linkage_key.return_value = LinkageKey(
        version=current_version, secret=secrets[current_version]
    )
    keys.get_linkage_key_by_version.side_effect = lambda v: LinkageKey(
        version=v, secret=secrets[v]
    )
...
class TestForNewSession:
    def test_returns_version_and_locator(self) -> None:
        svc = _make_service()
        result = svc.for_new_session("sess-1")
    ...
    def test_deterministic(self) -> None:
        ...
        a = svc.for_new_session("sess-1")
        b = svc.for_new_session("sess-1")
        assert a == b
...
class TestForExistingSession:
    def test_returns_same_locator_as_new(self) -> None:
        secret = os.urandom(32)
        svc = _make_service(secrets={2: secret})
        new = svc.for_new_session("sess-1")
        existing = svc.for_existing_session("sess-1", 2)
    ...
    def test_uses_stored_version(self) -> None:
        ...
        loc_v1 = svc.for_existing_session("sess-1", 1)
        loc_v2 = svc.for_existing_session("sess-1", 2)
...
class TestAnswerLocator:
    def test_returns_32_bytes(self) -> None:
        svc = _make_service()
        loc = svc.answer_locator("sess-1", "q-1", 2)
    ...
    def test_deterministic(self) -> None:
        secret = os.urandom(32)
        svc = _make_service(secrets={2: secret})
        a = svc.answer_locator("sess-1", "q-1", 2)
        b = svc.answer_locator("sess-1", "q-1", 2)
        assert a == b
    ...
    def test_different_questions_differ(self) -> None:
        ...
        svc = _make_service(secrets={2: secret})
        a = svc.answer_locator("sess-1", "q-1", 2)
        b = svc.answer_locator("sess-1", "q-2", 2)
        assert a != b
...
class TestAnswerLocators:
    def test_returns_map(self) -> None:
        secret = os.urandom(32)
        svc = _make_service(secrets={2: secret})
        result = svc.answer_locators("sess-1", ["q-1", "q-2", "q-3"], 2)
    ...
    def test_matches_single_answer_locator(self) -> None:
        secret = os.urandom(32)
        svc = _make_service(secrets={2: secret})
        batch = svc.answer_locators("sess-1", ["q-1"], 2)
        single = svc.answer_locator("sess-1", "q-1", 2)
        assert batch["q-1"] == single

    def test_fetches_key_once(self) -> None:
        svc = _make_service()
        svc.answer_locators("sess-1", ["q-1", "q-2", "q-3"], 2)
        svc._keys.get_linkage_key_by_version.assert_called_once_with(2)
