...
tags: [meta]
related_code: []
related_docs: ["Engineering migrations"]
---

# Documentation system migration

This manifest is the single source of truth for moving the 75 legacy documents
under `docs/` into the two-collection tree under `docs-new/` (which becomes
`docs/` at cutover). Every legacy document appears **exactly once** with one
destination and one action, so migration never duplicates content
opportunistically.
