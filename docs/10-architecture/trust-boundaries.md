...
| Image publisher workflow to ECR | Backend, Caddy, Squid, and Alloy image layers and manifests | A branch-restricted GitHub OIDC role receives publish and digest-inspection operations only on the four staging repositories; runtime pull policies are split by host responsibility; publication cannot promote SSM values | The checked-in manual workflow and IAM boundary have not yet proved a live hosted push; a partial publication can leave inert unpromoted images |
