...
| Local container environment    | `infra/docker/docker-compose.*.yml`, `infra/docker/*.Dockerfile`, `infra/docker/caddy/`, `infra/docker/postgres/`                                                 | Integrated local and local-proof container configuration.                                    |
...
| Rehearsal environment          | `infra/environments/rehearsal/compose/`, `infra/environments/rehearsal/fixtures/`, `infra/rehearsal/proxmox/cloud-init/`                                          | Rehearsal overrides and fixture configuration.                                               |
...
| Machine images                 | `infra/images/packer/*.pkr.hcl`, `infra/images/packer/variables/*.example`                                                                                        | Packer plugins, variables, locals, sources, builds, and copy-to-local variable examples.     |
| AWS CDK                        | `infra/cdk/cdk.json`, `infra/cdk/cdk.context.json`, `infra/cdk/flowform_infra/config/environments.py`, `infra/cdk/pyproject.toml`, `infra/cdk/pyrightconfig.json` | CDK entry configuration, cached lookup context, environment definitions, and Python tooling. |
...
| -------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `infra/cdk/.env.dev.example`                                   | Gitignored `infra/cdk/.env.<env>` files consumed by CDK configuration and its seeding helper. |
| `infra/images/packer/variables/aws.auto.pkrvars.hcl.example`   | Local AWS Packer variable file.                                                               |
| `infra/images/packer/variables/local.auto.pkrvars.hcl.example` | Local Proxmox Packer variable file.                                                           |
...
| `infra/rehearsal/proxmox/cloud-init/*.user-data.yaml`          | Rehearsal template-build cloud-init inputs.                                                   |
