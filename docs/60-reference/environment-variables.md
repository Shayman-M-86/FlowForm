...
| Runtime Compose | `BACKEND_IMAGE`, `CADDY_IMAGE`, `ALLOY_IMAGE`, `API_DOMAIN`, `APP_PRIVATE_IP`, `PROXY_PRIVATE_IP`, `SQUID_*`, `GRAFANA_CLOUD_LOKI_*`, `GRAFANA_CLOUD_TEMPO_*`, `OTEL_*` | Shared runtime Compose interpolation. Bootstrap-generated `/opt/flowform/*.env` files supply deployed values; Caddy's `OTEL_*` values are set directly by Compose. |
