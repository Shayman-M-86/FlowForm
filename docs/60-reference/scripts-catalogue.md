...
| Runtime bootstrap       | `infra/deployment/bootstrap/bootstrap-app.sh`, `infra/deployment/bootstrap/bootstrap-proxy.sh`, `infra/deployment/bootstrap/bootstrap-db.sh`                           | App, proxy, and rehearsal DB host bootstrap entry points.               |
| CDK support             | `infra/deployment/aws/scripts/seed-secrets.sh`                                                                                                   | AWS CDK environment seeding helper.                                     |
| Packer image build      | `infra/images/scripts/image`, `infra/images/scripts/lib/`, `infra/images/packer/provisioners/` | Single dispatcher for source preparation, selected AWS/Proxmox builds, fixture preload, verification, artifacts, and CDK-oriented AMI publication. |
| Image validation        | `infra/images/scripts/image verify`, `infra/tests/images/validate.sh`, `infra/tests/images/` | Packer syntax, dispatcher contracts, source preparation, live Proxmox disk limits, and AWS AMI/snapshot checks. |
| Proxmox deployment      | `infra/deployment/proxmox/host/setup-host.sh`, `infra/deployment/proxmox/terraform/`, `infra/deployment/proxmox/scripts/`                         | One-time host bridge setup, Terraform-managed rehearsal VMs/snippets, and operator helpers. |
| Rehearsal fixtures      | `infra/containers/strategies/rehearsal/services/localstack/seed-localstack.sh`, `infra/containers/strategies/rehearsal/services/registry/build-and-push-backend.sh` | Rehearsal fixture preparation.                                          |
