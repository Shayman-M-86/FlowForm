...
Response database tables for anonymous encrypted envelopes, logical answers,
and immutable answer revisions.

This database has no foreign keys back to the core database. Cross-database
lookup values are HMAC-derived locators, and plaintext respondent identity does
not live here.
