...
CREATE TABLE users (
    ...
    -- Email is NOT globally unique: Auth0 can issue several identities (Google,
    -- password, passwordless) sharing one email. Login identity is auth0_user_id.
    -- uq_users_id_email below keeps (id, email) unique so the composite FK from
    -- project_subject_identities stays valid; email alone is only indexed for lookup.
    email TEXT NOT NULL,
    display_name TEXT,
...
-- Email is no longer unique; keep it indexed for invite-by-email lookups.
...
-- =========================================
-- Core identity, session, and cryptographic-linkage tables; see
-- docs/project-knowledge/data/responses-and-encryption.md.
