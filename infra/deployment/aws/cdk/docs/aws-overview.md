...
[`infra/deployment/aws/cdk/`](..), using AWS CDK (Python). Goal: if the AWS account
...
- **Local development** — Flask, React, Postgres, PgBouncer all run in
  Docker Compose (`infra/containers/runtime/development/compose/`), untouched
  by this.
...
from the gitignored `infra/deployment/aws/cdk/.env.<env>` file — the frontend stack fails
...
(`infra/env/dev/.backend.env`). See [`environments.md`](environments.md) for
