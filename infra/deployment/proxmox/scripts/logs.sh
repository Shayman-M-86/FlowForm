...
#
# Usage:
#   logs.sh [app|proxy|registry|db] [options] [-- <extra docker logs args>]
#
...
#
# Examples:
#   logs.sh app -f                     # tail the backend
#   logs.sh app -e -n 200              # recent failures only
#   logs.sh app -r 350889d7-7602-...   # one request end to end
#   logs.sh app --raw -n 5             # full tracebacks
#   logs.sh proxy -c flowform-proxy-squid-1 -f
#   logs.sh db -f                      # tail PostgreSQL on VM 240
...
# Private addresses are fixed in terraform/virtual-machines.tf; keep in sync with the
...
target_ip() {
  case "$1" in
    ...
    db)       echo "10.10.10.40" ;;
    *)        die "unknown target: $1 (expected app, proxy, registry, or db)" ;;
  esac
}
...
case "${1:-}" in
  app|proxy|registry|db) TARGET="$1"; shift ;;
esac
