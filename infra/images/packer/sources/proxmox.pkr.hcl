...
source "proxmox-clone" "db_fixture" {
  proxmox_url              = var.proxmox_api_url
  node                     = var.proxmox_node
  username                 = var.proxmox_token_id
  token                    = var.proxmox_token_secret
  ...
  clone_vm             = var.proxmox_golden_template != "" ? var.proxmox_golden_template : var.proxmox_template_name
  full_clone           = true
  vm_id                = var.proxmox_db_fixture_vm_id
  vm_name              = var.proxmox_db_fixture_template_name
  template_name        = var.proxmox_db_fixture_template_name
  template_description = "FlowForm Proxmox PostgreSQL offline fixture derived from the golden template"
