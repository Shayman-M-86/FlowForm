...
SHARED_SCRIPT_DIR="${FLOWFORM_SHARED_SCRIPT_DIR:-/opt/flowform/host/bin}"

# Pull in the shared library before any log/die call.
# shellcheck source=/opt/flowform/host/bin/bootstrap-common.sh
...
COMPOSE_FILE="${COMPOSE_FILE:-/opt/flowform/runtime/compose/proxy.yml}"
# AWS service configuration is embedded in the container images, so it needs
# no host bind-mount override. Rehearsal explicitly supplies its own override.
...
render_proxy_env() {
  ...
  app_source_ip="$(getent ahostsv4 "${APP_UPSTREAM_HOST:-${APP_PRIVATE_IP}}" \
    | awk 'NR == 1 { print $1; exit }')"
  [[ "${app_source_ip}" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]] \
    || die "could not resolve app private DNS: ${APP_UPSTREAM_HOST:-${APP_PRIVATE_IP}}"

  # Squid receives the currently resolved app address, while Caddy retains the
  # stable private DNS name.
  {
    printf 'PROXY_PRIVATE_IP=%s\n' "${PROXY_PRIVATE_IP}"
    printf 'APP_PRIVATE_IP=%s\n'   "${APP_PRIVATE_IP}"
    printf 'APP_UPSTREAM_HOST=%s\n' "${APP_UPSTREAM_HOST:-${APP_PRIVATE_IP}}"
    printf 'SQUID_APP_SOURCE_CIDR=%s/32\n' "${app_source_ip}"
    printf 'FLOWFORM_ENV=%s\n' "${FLOWFORM_ENV}"
    printf 'FLOWFORM_PLATFORM=%s\n' "${FLOWFORM_PLATFORM}"
