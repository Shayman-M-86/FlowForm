...
image_aws_manifest() { # base|app|proxy
  case "$1" in
    base|app|proxy) printf '%s/manifests/%s-manifest.json' "${PACKER_DIR}" "$1" ;;
    *) die "AWS image role must be base, app, or proxy" ;;
  esac
}

image_aws_artifact_id() { # role [manifest]
  local role="$1" manifest="${2:-}" ami_id
  manifest="${manifest:-$(image_aws_manifest "${role}")}"
  require_command jq
  require_file "${manifest}" "Run 'infra/machine-images/tooling/image build aws ${role}' first"
  ami_id="$(jq -er '
    .builds | map(select(.builder_type == "amazon-ebs")) | last | .artifact_id // empty
    | select(type == "string") | split(":") | .[1] // empty
  ' "${manifest}")" || die "no amazon-ebs artifact found in ${manifest}; run 'infra/machine-images/tooling/image build aws ${role}' first"
  [[ "${ami_id}" =~ ^ami-[0-9a-f]+$ ]] || die "invalid AWS AMI ID in ${manifest}: ${ami_id}"
  printf '%s' "${ami_id}"
}

image_cdk_ami_parameter() { # environment role
  local environment="$1" role="$2"
  [[ "${environment}" =~ ^(dev|staging|prod)$ ]] || die "AWS environment must be dev, staging, or prod"
  case "${role}" in
    base) printf '/flowform/%s/ec2/baseAmiId' "${environment}" ;;
    app) printf '/flowform/%s/ec2/appAmiId' "${environment}" ;;
    proxy) printf '/flowform/%s/ec2/proxyAmiId' "${environment}" ;;
    *) die "AWS image role must be base, app, or proxy" ;;
  esac
}
