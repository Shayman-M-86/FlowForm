...
- Use PostgreSQL identity columns for primary keys: `id BIGINT GENERATED IDENTITY PRIMARY KEY` (preferred over `BIGSERIAL`).
...
class ProjectMembership(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True)  # uses DB identity (GENERATED AS IDENTITY)
