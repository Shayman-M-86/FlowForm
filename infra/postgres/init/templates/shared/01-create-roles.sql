...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = '${FF_PGDB_RESPONSE__APP_USER}') THEN
        CREATE ROLE ${FF_PGDB_RESPONSE__APP_USER}
        LOGIN
        PASSWORD '${FF_PGDB_RESPONSE__APP_PASSWORD}';
    END IF;
END
$$;
...
ALTER ROLE ${FF_PGDB_RESPONSE__APP_USER}
    NOSUPERUSER
    NOCREATEDB
    NOCREATEROLE
    NOREPLICATION
...
GRANT CONNECT ON DATABASE ${FF_PGDB_RESPONSE__DB_NAME}
