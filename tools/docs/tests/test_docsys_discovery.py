...
from docsys.contracts import (
    FindRequest,
    ReadRequest,
    execute_find,
    execute_read,
)
from docsys.model import DOCS, ROOT, DocSet, Document
...
class DiscoveryContractTests(unittest.TestCase):
    ...
    def test_match_modes_and_exact_code_filter_are_explicit(self) -> None:
        code_path = "tools/docs/docsys/model.py"
        docs = [
            _doc(
                "both",
                title="Alpha Beta",
        ...
        with self.assertRaisesRegex(ValueError, "exact file"):
            FindRequest(code_paths=("tools/docs/docsys",))
