...
- A `.env` file at `tools/.env` with your Auth0 settings:
...
```bash
tools/bin/flowform-openapi login
```

To bypass Auth0 entirely (e.g. pasting a token grabbed from the browser):

```bash
FLOWFORM_DEV_TOKEN=eyJhbGc... tools/bin/flowform-openapi
```
  ...
  "mcpServers": {
    "flowform-openapi": {
      ...
      "args": [
        ...
        "--with", "httpx",
        "python",
        "/absolute/path/to/FlowForm/tools/bin/flowform-openapi"
      ],
      "env": {
