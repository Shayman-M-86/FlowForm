...
def _open_browser(url: str) -> None:
    ...
    if override:
        ...
        try:
            ...
            print(f"[flowform-openapi] FLOWFORM_BROWSER_CMD failed: {exc}", file=sys.stderr)
    ...
    if _is_wsl():
        ...
        try:
            ...
            print(
                f"[flowform-openapi] Could not launch Windows browser via cmd.exe ({exc}); "
...
def _encryption_key() -> bytes:
    ...
    if value.startswith("base64:"):
        ...
        try:
            ...
            print(
                f"[flowform-openapi] {TOKEN_ENCRYPTION_KEY_ENV} is not valid base64.",
                file=sys.stderr,
            )
            raise SystemExit(9) from exc
        if len(key) != 32:
            print(
                f"[flowform-openapi] {TOKEN_ENCRYPTION_KEY_ENV} must decode to 32 bytes.",
                file=sys.stderr,
            )
            raise SystemExit(9)
        return key
...
def _decrypt_secret(cfg: "Auth0Config", key: str, value: str) -> str:
    ...
    try:
        ...
        print(
            f"[flowform-openapi] Could not decrypt cached token. Check {TOKEN_ENCRYPTION_KEY_ENV}.",
            file=sys.stderr,
        )
        raise SystemExit(9) from exc
...
def _require_env(name: str) -> str:
    ...
    if not value:
        print(
            f"[flowform-openapi] Missing required env var: {name}. "
            "Set it in tools/.env.",
            file=sys.stderr,
        )
        raise SystemExit(2)
...
def _device_authorize(cfg: Auth0Config) -> dict[str, Any]:
    ...
    if response.status_code >= 400:
        ...
        print(
            f"[flowform-openapi] Auth0 rejected the device authorization request "
            f"(HTTP {response.status_code}).",
            file=sys.stderr,
        )
        print(f"[flowform-openapi] Auth0 response: {body}", file=sys.stderr)
        print(
            f"[flowform-openapi] Request was: domain={cfg.domain} "
            f"client_id={cfg.client_id} audience={cfg.audience} scope={SCOPE!r}",
            file=sys.stderr,
        )
        print(
            "[flowform-openapi] Common causes:\n"
...
def _poll_for_token(cfg: Auth0Config, device_code: str, interval: int) -> dict[str, Any]:
    ...
    while True:
        ...
        if error == "expired_token":
            print(
                "[flowform-openapi] Device code expired before login completed. "
                "Run again to start a new login.",
                file=sys.stderr,
            )
            raise SystemExit(4)
        if error == "access_denied":
            print("[flowform-openapi] Login was denied.", file=sys.stderr)
            raise SystemExit(5)

        print(
            f"[flowform-openapi] Unexpected token endpoint response "
...
def _run_device_flow(cfg: Auth0Config) -> dict[str, Any]:
    ...
    print("[flowform-openapi] Opening your browser to log in...", file=sys.stderr)
    print(f"[flowform-openapi] URL:       {verification_uri}", file=sys.stderr)
    print(f"[flowform-openapi] User code: {user_code}", file=sys.stderr)
    print(
        "[flowform-openapi] If the browser doesn't open, paste the URL above "
    ...
    print("[flowform-openapi] Login complete. Token cached.", file=sys.stderr)
...
def login_cli() -> None:
    ...
    print(
        f"[flowform-openapi] Cached token metadata at {_token_path()}",
        file=sys.stderr,
    )
...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point: ``python -m flowform_tools.openapi.auth login``."""
    args = sys.argv[1:] if argv is None else argv
    if args and args[0] == "login":
        login_cli()
    else:
        print("Usage: python -m flowform_tools.openapi.auth login", file=sys.stderr)
        raise SystemExit(2)
