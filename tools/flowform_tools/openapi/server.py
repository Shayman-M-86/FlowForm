...
Run:
    ...
    tools/bin/flowform-openapi

Configuration via env (typically in tools/.env):
...
from flowform_tools.openapi.auth import get_access_token
...
def _fetch_spec() -> dict[str, Any]:
    try:
        ...
        print(
            f"[flowform-openapi] Failed to fetch OpenAPI spec from {SPEC_URL}: {exc}",
            file=sys.stderr,
        )
        print(
            "[flowform-openapi] Start the backend with `uv run flask run` from backend/ first.",
            file=sys.stderr,
        )
